...
- The selected real-time target is a perceptual codec-rate FM kernel with
  ...
  **326.27-cycle** budget — full decoded control plus envelope curvature
  ...
  work: the feedback-0 trajectory anomaly and algorithm-7's dropped
  feedback (the two honest topology failures under the two-tier relative
  gate in docs/perceptual-compatibility.md), exact per-level feedback
  depth pending a render restructure, DT1/DT2, noise-frequency/output
  substitution, and sub-block event splitting.
