...
; silence, counts transmit word slots and frame syncs, and reports its
; counters over the host port. A frame counter that never advances therefore
; means the crossbar delivers no SSI clock at the connected prescale - the
; exact real-hardware question the polled design isolates from the
; production kernel.
;
; The first physical-Falcon run (2026-09-02) found exactly that: no frames at
; any prescale with the bare bring-up, while the production player, which
; pins the whole sound matrix and only arms its transmitter after Devconnect,
; does play. The host side now bisects the bring-up, and this program gained
; the two commands that needs: a word-slot counter beside the frame counter,
; so a dead clock is told apart from a transmitter that runs without frame
; syncs, and a transmitter re-arm the host issues after Devconnect.
;
...
RT_CMD_SSI_RESTART equ     $060000         ; disable, reprogram, re-enable TX
RT_CMD_READ_SR     equ     $070000         ; reply = SSISR as the DSP sees it
...
; then writing TX is also the TUE-clearing sequence. Every service is one
; word slot; TFS marks exactly one service per stereo frame, so counting
; TFS-flagged services counts frames; TUE-flagged services mean this loop
; missed a word slot and the host-side report flags the run as suspect.
rate_loop:
        jclr    #6,x:m_sr,rate_host     ; TDE: a word slot wants data
        movep   x:m_sr,x:RT_STATUS_COPY
        movep   y0,x:m_tx
        move    x:RT_WORD_COUNT,a
        add     x1,a
        move    a1,x:RT_WORD_COUNT
...
rate_host:
        ...
        cmp     x0,a
        jeq     rate_cmd_reset
        move    #>RT_CMD_READ_WORDS,x0
        cmp     x0,a
        jeq     rate_cmd_words
        move    #>RT_CMD_SSI_RESTART,x0
        cmp     x0,a
        jeq     rate_cmd_restart
        move    #>RT_CMD_READ_SR,x0
        cmp     x0,a
        jeq     rate_cmd_sr
...
; Program the transmitter from scratch: 16-bit words, two-word network
; frame, transmit enabled, polled (the production $5a00 without TIE). Run at
; boot and again on RT_CMD_SSI_RESTART, so the host can re-arm the
; transmitter after the crossbar clock exists.
rate_ssi_arm:
        movep   #0,x:m_crb
        movep   #$4100,x:m_cra
        movep   #$1a00,x:m_crb
