...
; player assumes (24,584.9609375 Hz at prescale 3). Results are printed as
; deterministic PASS/FAIL lines and duplicated in RATETEST.TXT beside the
; program, so a real-Falcon run can be reported as one file or photo.
...
measure_one:
        ...
        ; let the new prescale settle before opening the window
        bsr     get_ticks
        move.l  d0,d3
.settle:
        bsr     get_ticks
        ...
        ; open the window on a fresh tick edge, then snapshot both DSP
...
.window:
        ...
        ; 24-bit modular counter deltas over the exact measured tick count
        move.l  meas_c1,d0
        sub.l   meas_c0,d0
        andi.l  #$00ffffff,d0
...
txt_sound:         dc.b 'sound lock/route  ',0
